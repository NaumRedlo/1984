CATALOG = {
    ...
    "help.sec.osu.body": {
        "en": ("<b>Basic commands</b>\n\n"
               ...
               "<code>rs</code> — last played beatmap, any mode (<code>rs taiko</code>, <code>rs m</code> for one)\n"
               "<code>tpp</code> — your top plays\n"
               "<code>tp [N] [name]</code> — top play #N as a card (<code>tp 3</code>, <code>tp mania 1</code>)\n"
               "<code>update [name]</code> — what changed since the last update: pp, rank and new top plays\n"
               "<code>cmp [name]</code> — compare your statistics with a player\n"
               "<code>lb</code> — group leaderboard\n"
               "<code>rk [mode]</code> — the group ranked by pp in osu!, taiko, catch or mania\n"
               "<code>find &lt;nickname&gt;</code> — who in this group linked that osu! account\n"
        ...
        "ru": ("<b>Основные команды</b>\n\n"
               ...
               "<code>rs</code> — последняя сыгранная карта в любом режиме (<code>rs taiko</code>, <code>rs m</code> — в одном)\n"
               "<code>tpp</code> — твои топ-плеи\n"
               "<code>tp [N] [ник]</code> — топ-плей №N карточкой (<code>tp 3</code>, <code>tp mania 1</code>)\n"
               "<code>update [ник]</code> — что изменилось с прошлого раза: pp, ранг и новое в топе\n"
               "<code>cmp [ник]</code> — сравнить статистику с игроком\n"
               "<code>lb</code> — лидерборд беседы\n"
               "<code>rk [режим]</code> — рейтинг беседы по pp в osu!, taiko, catch или mania\n"
               "<code>find &lt;ник&gt;</code> — кто в беседе привязал этот osu!-аккаунт\n"
