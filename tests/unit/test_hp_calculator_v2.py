"""Tests for HPS v2 (Math Manifest) — calculate_hps_v2 and its parts."""
...
class TestRanksV2:
    def test_candidate_floor(self):
        assert get_rank_for_hp_v2(0) == "Candidate"
        assert get_rank_for_hp_v2(249) == "Candidate"

    def test_party_member(self):
        assert get_rank_for_hp_v2(250) == "Party Member"
        assert get_rank_for_hp_v2(749) == "Party Member"

    def test_inspector(self):
        assert get_rank_for_hp_v2(750) == "Inspector"
        assert get_rank_for_hp_v2(1499) == "Inspector"

    def test_high_commissioner(self):
        assert get_rank_for_hp_v2(1500) == "High Commissioner"
        assert get_rank_for_hp_v2(2999) == "High Commissioner"

    def test_big_brother(self):
        assert get_rank_for_hp_v2(3000) == "Big Brother"
        assert get_rank_for_hp_v2(999_999) == "Big Brother"

    def test_thresholds_table_sorted_descending(self):
        # Failsafe — get_rank_for_hp_v2 relies on the list being sorted high→low.
        thresholds = [t for t, _ in RANK_THRESHOLDS_V2]
        assert thresholds == sorted(thresholds, reverse=True)

    def test_next_rank_info_at_candidate(self):
        info = get_next_rank_info_v2(0)
        assert info["current"] == "Candidate"
        assert info["next"] == "Party Member"
        assert info["hp_needed"] == 250

    def test_next_rank_info_at_top(self):
        info = get_next_rank_info_v2(5000)
...
class TestCalculateHpsV2:
    ...
    def test_vanguard_added_only_when_first(self):
        ...
        no_vg = calculate_hps_v2(**common, is_first_submission=False)
        vg    = calculate_hps_v2(**common, is_first_submission=True)
        assert vg["final_hp"] - no_vg["final_hp"] == HPS_V2_VANGUARD
        assert no_vg["vanguard"] == 0
        assert vg["vanguard"] == HPS_V2_VANGUARD
    ...
    def test_win_pays_more_than_condition(self):
        ...
        win  = calculate_hps_v2(result_type="win", **common)
        cond = calculate_hps_v2(result_type="condition", **common)
    ...
    def test_skewed_map_skewed_player(self):
        ...
        result_bad  = calculate_hps_v2(result_type="condition", map_info=map_info,
                                       player_skill=bad_speed,  score=score)
        result_good = calculate_hps_v2(result_type="condition", map_info=map_info,
    ...
    def test_ur_override_skips_recompute(self):
        ...
        a = calculate_hps_v2(score=ScoreStats(0, 0, 0, misses=0, combo=1000), **common)
        b = calculate_hps_v2(score=ScoreStats(500, 0, 0, misses=0, combo=1000), **common)
    ...
    def test_base_default_is_60(self):
        # Sanity: HPS_V2_BASE matches the Manifest default and the formula
        # actually uses it.
        assert HPS_V2_BASE == 60
        result = calculate_hps_v2(
            result_type="condition",
            map_info=_balanced_map(),
            player_skill=_flat_player(),
