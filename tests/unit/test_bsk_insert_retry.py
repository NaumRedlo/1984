"""Unit tests for services.bsk.bulk_import._insert_bsk_map retry logic.
...
import services.bsk.bulk_import as bi
...
def _fast_and_isolated(monkeypatch):
    ...
    monkeypatch.setattr(bi, "BSK_BULK_DB_RETRY_DELAY", 0)
    monkeypatch.setattr("services.bsk.map_pool.apply_to_entry", lambda e, r: None)
...
def _run(monkeypatch, commit_effects):
    ...
    status, _entry = asyncio.run(bi._insert_bsk_map({"beatmap_id": 1}, {}))
...
def test_gives_up_after_max_retries(monkeypatch):
    # Always locked → 'locked' after exactly BSK_BULK_DB_RETRIES attempts.
    status, commits = _run(monkeypatch, [_locked()] * 10)
    assert status == "locked"
    assert commits == bi.BSK_BULK_DB_RETRIES
