"""Unit tests for services.hps.bsk_user_skill.

Each test spins up an in-memory aiosqlite database with the bare schema we
exercise (users, bounties, submissions, bsk_map_pool) and seeds rows by hand.
...
from services.hps.bsk_user_skill import (
    AXES,
    BOOTSTRAP_FULL_N,
    NEUTRAL_DEFAULT,
    TIME_DECAY_DAYS,
    _bsk_user_pp_prior,
    _c_pen,
    compute_bsk_user_skill,
    refresh_bsk_user_skill,
)
...
class TestPpPrior:
    def test_zero_pp(self):
        assert _bsk_user_pp_prior(0) == NEUTRAL_DEFAULT
        assert _bsk_user_pp_prior(None) == NEUTRAL_DEFAULT

    def test_1000pp_equals_4(self):
        # (1000/1000)^0.6 + 3 = 1 + 3 = 4
        assert _bsk_user_pp_prior(1000) == pytest.approx(4.0)
    ...
    def test_top_player_spread(self):
        ...
        ten_k = _bsk_user_pp_prior(10_000)
        twenty_k = _bsk_user_pp_prior(20_000)
    ...
    def test_clamped_at_10(self):
        ...
        assert _bsk_user_pp_prior(10_000_000) == 10.0
...
class TestComputeBskUserSkill:
    ...
    async def test_map_not_in_pool_uses_sr_fallback(self, session):
        # No bsk_map_pool row → all axes use bounty.star_rating.
        user = await _add_user(session, pp=10)
...
class TestRefreshBskUserSkill:
    @pytest.mark.asyncio
    async def test_writes_back_to_user(self, session):
        user = await _add_user(session, pp=1000)
        await refresh_bsk_user_skill(user, session)
        assert user.bsk_user_aim == pytest.approx(4.0)
        assert user.bsk_user_speed == pytest.approx(4.0)
        assert user.bsk_user_acc == pytest.approx(4.0)
        assert user.bsk_user_cons == pytest.approx(4.0)
        assert user.bsk_skill_calculated_at is not None
