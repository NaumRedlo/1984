...
Triggers: duel, duelresult/dr, duelstats/ds, duelcancel/dc
...
async def cmd_duel(message: types.Message, trigger_args: TriggerArgs, osu_api_client):
    ...
    async with get_db_session() as session:
        ...
        target = await _resolve_target(session, target_name)
        if not target:
            await message.answer(
                f"Игрок <b>{escape_html(target_name)}</b> не найден или не зарегистрирован.",
                parse_mode="HTML",
            )
            return
    ...
    await message.answer(
        ...
        f"<i>{escape_html(p2_name)}, принимаешь вызов?</i>",
...
async def on_duel_custom(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Введи ID карты:")
    await callback.answer()
...
async def _resolve_target(session, target_name: str):
    """Resolve target by osu username (case insensitive)."""
    from sqlalchemy import select, func
    stmt = select(User).where(func.lower(User.osu_username) == target_name.lower())
