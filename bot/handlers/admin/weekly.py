...
async def cmd_set_weekly_chat(message: types.Message):
    ...
    async with get_db_session() as session:
        stmt = sqlite_insert(BotSettings).values(key="weekly_chat_id", value=chat_id)
        stmt = stmt.on_conflict_do_update(index_elements=["key"], set_={"value": chat_id})
        await session.execute(stmt)
        await session.commit()
    await message.answer(
        format_success(f"Еженедельная рассылка баунти настроена на этот чат ({chat_id})."),
        parse_mode="HTML",
    )
...
async def cmd_send_weekly(message: types.Message):
    ...
    try:
        await send_weekly_digest(message.bot, chat_id)
