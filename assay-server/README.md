...
| `POST /v1/strains` | график сложности: самый трудный объект каждых 400 мс игры (aim + speed в osu!, все навыки в остальных режимах), от 0 до 1, усреднённый до `points` значений (по умолчанию 64) |
...
- Режимы (`"ruleset": 0..3`): osu!, taiko, catch, mania для звёзд, pp с готовой статистикой и графика, в том числе на конвертах; для mania в `attributes` есть `key_count`. «Если FC/SS» и what-if — только osu!.
...
Сервис — единственный источник pp в боте: пока `ASSAY_URL` пуст или сервис не отвечает, на карточках только pp и звёзды из osu!, без «если FC/SS», графика и what-if.
...
3. **В самом боте** каждый ранкнутый скор, для которого osu! прислал pp, сравнивается с посчитанным;
   расхождение больше 1% пишется в лог как `pp drift`.
