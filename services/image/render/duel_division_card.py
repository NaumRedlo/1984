"""DUEL division change card renderer."""
import asyncio
...
from services.image.constants import (
    ...
    ACCENT_RED, ACCENT_GREEN, PADDING_X, DUEL_DIVISION_COLORS, DUEL_DIVISION_GRADIENTS,
)
...
class DuelDivisionCardMixin:

    def generate_duel_division_card(
        self,
        data: Dict,
        ...
        new_color = DUEL_DIVISION_COLORS.get(new_div_base, TEXT_PRIMARY)
        grad_pair = DUEL_DIVISION_GRADIENTS.get(new_div_base, (new_color, TEXT_PRIMARY))
        ...
        if avatar:
            ...
            self._aa_rounded_outline(
                img,
                (avatar_x, avatar_y, avatar_x + avatar_size, avatar_y + avatar_size),
                radius=12, outline=ACCENT_RED, width=2, fill=(50, 50, 70),
            )
        draw = ImageDraw.Draw(img)
        ...
        # ── Bottom RIGHT: DUEL points ──────────────────────────────────────────
        right_x = half_w + PADDING_X
        duel_points = data.get("duel_points")

        if duel_points is not None:
            label_str = "Conservative"
            val_str = f"{duel_points:.0f}"
    ...
    async def generate_duel_division_card_async(self, data: Dict) -> BytesIO:
        ...
        return await asyncio.to_thread(self.generate_duel_division_card, data, avatar, cover)
