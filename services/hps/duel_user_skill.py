"""Per-axis DUEL_user skill score for the HPS Ψ(Δ) module.

Each user has four scalar skill values in [0..10] — aim/speed/acc/cons — used
by the HPS payout to compute Δ = DUEL_map − DUEL_user.  The HPS Manifest defines
these as the **weighted average DUEL-stars of the user's top-10 successful
submissions in the last 90 days**, per axis.

Implementation details agreed upon in design:
  * Bootstrap for thin histories: `DUEL_user_pp = clamp((pp/1000)^0.6 + 3, 0, 10)`,
  ...
  * Maps absent from `duel_map_pool` contribute via a fallback: all four axis
...
for committing.  `refresh_duel_user_skill` is the convenience wrapper that
...
def _duel_user_pp_prior(player_pp: Optional[int]) -> float:
...
async def compute_duel_user_skill(
    user: User,
    session: AsyncSession,
    *,
    as_of: Optional[datetime] = None,
) -> DuelUserSkill:
    """Calculate DUEL_user without touching the User row.

    Pure compute path — the caller decides whether to persist (see
    `refresh_duel_user_skill`).  Splitting this out also makes it usable from
    ...
    ones — useful for dry-run, where we want to know "what DUEL_user was when
    this score landed", not "what it is today after that score and everything
    since".
    """
    pp_prior = _duel_user_pp_prior(user.player_pp)
    ...
    pool_rows: dict[int, DuelMapPool] = {}
    if beatmap_ids:
...
async def refresh_duel_user_skill(user: User, session: AsyncSession) -> DuelUserSkill:
    """Compute and write DUEL_user back to the user row.

    Caller still needs to commit.  Updates `duel_skill_calculated_at` so we can
    detect stale rows in a periodic refresher.
    """
    skill = await compute_duel_user_skill(user, session)
    user.duel_user_aim   = skill.aim
    user.duel_user_speed = skill.speed
    user.duel_user_acc   = skill.acc
    user.duel_user_cons  = skill.cons
    user.duel_skill_calculated_at = datetime.now(timezone.utc).replace(tzinfo=None)
    return skill
