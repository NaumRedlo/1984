...
class DuelManager:
    ...
    def __init__(self, osu_api, session_factory):
        ...
        self._result_locks: Dict[int, asyncio.Lock] = {}
    ...
    def _pick_turn_user_id(self, duel_id: int) -> Optional[int]:
        state = self._by_duel.get(duel_id)
        if not state:
            return None
        return state.player1_user_id if state.current_round % 2 == 0 else state.player2_user_id

    def _pick_turn_name(self, duel_id: int) -> str:
        state = self._by_duel.get(duel_id)
        if not state:
            return "—"
        return state.player1_name if state.current_round % 2 == 0 else state.player2_name

    def build_map_pick_cards(self, duel_id: int, suggestions: List[Dict]) -> List[Dict]:
        state = self._by_duel.get(duel_id)
        if not state:
            return []
        ...
        for idx, suggestion in enumerate(suggestions, start=1):
            cards.append({
                "index": idx,
                "pick_turn": self._pick_turn_name(duel_id),
                "beatmap_id": suggestion.get("beatmap_id", 0),
                "title": suggestion.get("title", "Unknown"),
                "star_rating": suggestion.get("star_rating", 0.0),
                "mappool_size": len([m for m in state.mappool if m not in state.played_maps]) or len(state.mappool),
            })
        return cards
    ...
    async def check_results(self, duel_id: int, force_timeout: bool = False) -> Optional[Dict]:
        ...
        """
        lock = self._result_locks.setdefault(duel_id, asyncio.Lock())
        async with lock:
            state = self._by_duel.get(duel_id)
            if not state or not state.current_beatmap_id:
                return None
            if state.resolution_in_progress:
                return None
            state.resolution_in_progress = True

            try:
                beatmap_id = state.current_beatmap_id
                picked_at = state.round_picked_at
                deadline = picked_at + timedelta(seconds=ROUND_TIMEOUT) if picked_at else None
                ...
                p1_match = self._find_valid_score(p1_scores, beatmap_id, picked_at, deadline)
                p2_match = self._find_valid_score(p2_scores, beatmap_id, picked_at, deadline)
                ...
                state.resolution_in_progress = False
    ...
    async def get_completed_duel_stats(self, user_id: int, limit: int = 5) -> Dict:
        """Get completed duel stats and recent history from persisted duel records."""
        async with self._session_factory() as session:
            stmt = (
                select(Duel)
                .where(
                ...
                .order_by(Duel.completed_at.desc().nullslast(), Duel.id.desc())
            )
            ...
            formats_seen = []
            for duel in duels:
                if duel.winner_user_id == user_id:
                    wins += 1
                elif duel.winner_user_id:
                    losses += 1
                else:
                    draws += 1
                if duel.best_of not in formats_seen:
                    formats_seen.append(duel.best_of)

            recent_duels = duels[:limit]
            recent_ids = [duel.id for duel in recent_duels]
            round_counts: Dict[tuple, int] = {}
            if recent_ids:
                round_stmt = (
                    select(DuelRound.duel_id, DuelRound.winner_user_id, func.count(DuelRound.id))
                    .where(DuelRound.duel_id.in_(recent_ids))
                    .group_by(DuelRound.duel_id, DuelRound.winner_user_id)
                )
                round_result = await session.execute(round_stmt)
                for duel_id, winner_user_id, count in round_result.all():
                    round_counts[(duel_id, winner_user_id)] = count

            history: List[Dict] = []
            for duel in recent_duels:
                opponent_id = duel.player2_user_id if duel.player1_user_id == user_id else duel.player1_user_id
                opponent = await session.get(User, opponent_id)
                if duel.player1_user_id == user_id:
                    own_rounds = round_counts.get((duel.id, duel.player1_user_id), 0)
                    opp_rounds = round_counts.get((duel.id, duel.player2_user_id), 0)
                else:
                    own_rounds = round_counts.get((duel.id, duel.player2_user_id), 0)
                    opp_rounds = round_counts.get((duel.id, duel.player1_user_id), 0)
                ...
                history.append({
                    "opponent_name": opponent.osu_username if opponent else "—",
                    "result": result_text,
                    "best_of": duel.best_of,
                    "completed_at": duel.completed_at,
                    "score_line": f"{own_rounds:,}:{opp_rounds:,}",
                })
            ...
            return {
                "summary": {
                    ...
                    "win_rate": win_rate,
                    "formats": [f"BO{best_of}" for best_of in sorted(formats_seen)],
                },
                "duels": history,
            }

    async def get_completed_duel_history(self, user_id: int, limit: int = 5) -> List[Dict]:
        """Get completed duel history from persisted duel records."""
        stats = await self.get_completed_duel_stats(user_id, limit=limit)
        return stats.get("duels", [])
    ...
    async def _finalize_duel(self, state: DuelState):
        ...
        self._result_locks.pop(state.duel_id, None)
    ...
    async def get_duel_rounds(self, duel_id: int) -> List[Dict]:
        ...
        async with self._session_factory() as session:
            ...
            duel = await session.get(Duel, duel_id)
            player1 = await session.get(User, duel.player1_user_id) if duel else None
            player2 = await session.get(User, duel.player2_user_id) if duel else None
            ...
            for r in rounds:
                ...
                winner_player = 0
                if duel:
                    if r.winner_user_id == duel.player1_user_id:
                        rw_name = player1.osu_username if player1 else "—"
                        winner_player = 1
                    elif r.winner_user_id == duel.player2_user_id:
                        rw_name = player2.osu_username if player2 else "—"
                        winner_player = 2
                round_data.append({
    ...
    def _find_valid_score(
        scores: List[Dict], beatmap_id: int, picked_at: Optional[datetime], deadline: Optional[datetime] = None
    ) -> Optional[Dict]:
        ...
        - Rejects scores after the round deadline when deadline is provided
        - Rejects Relax-style and ScoreV2 submissions for duel resolution
        - Returns the earliest valid score (first attempt, no retry advantage)
        - Fails closed on malformed timestamps
        """
        forbidden_mods = {"RX", "AP", "AT", "SV2"}
        matches = []
        for s in scores:
            bm = s.get("beatmap", {})
            ...
            raw_mods = s.get("mods", []) or []
            mod_names = []
            for mod in raw_mods:
                if isinstance(mod, dict):
                    mod_names.append(str(mod.get("acronym", "")).upper())
                else:
                    mod_names.append(str(mod).upper())
            if any(mod in forbidden_mods for mod in mod_names):
                continue

            if picked_at:
                score_time_str = s.get("created_at") or s.get("ended_at")
                if not score_time_str:
                    continue
                try:
                ...
                if score_time < picked_at:
                    continue
                if deadline and score_time > deadline:
                    continue
