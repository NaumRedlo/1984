...
Дуэль идёт по принципу турнирного pool draft:

1. Сервер случайным образом отбирает **6 карт** из общего пула BSK.
2. Игроки **по очереди** банят карты до тех пор, пока не останется ровно одна — она и становится картой раунда.
3. Дуэли играются до победы в нескольких раундах; пул каждый раз пополняется новой случайной выборкой.
4. На время вызова приходит **пинг сопернику** в личку, ошибки Telegram (`BadRequest`) больше не ломают флоу.
5. Активные дуэли **переживают рестарт бота** — состояние раунда восстанавливается из БД, включая собранный пул карт.
